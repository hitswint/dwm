...
#define SPCW(X)                 (X->icon ? X->icw + ICONSPACING : 0)
...
enum { NetSupported, NetSystemTray, NetSystemTrayOP, NetSystemTrayOrientation,
       NetWMName, NetWMIcon, NetWMState, NetWMFullscreen, NetActiveWindow, NetWMWindowType,
...
static void focusstack_mon(const Arg *arg);
...
static void pushstack_mon(const Arg *arg);
...
static void tag_mon(const Arg *arg);
...
static void toggletag_mon(const Arg *arg);
static void toggleview(const Arg *arg);
static void toggleview_mon(const Arg *arg);
...
static void view_mon(const Arg *arg);
...
deck(Monitor *m) {
	...
	Client *c;
	Area *areas = m->pertag->areas[m->pertag->curtag];
        ...
        if ((w = x - xx) > bh) {
			...
			sel_max = w - unsel_max;
				...
				/* drw_text(drw, x, 0, w, bh, c->name, 0); */
                                drw_text(drw, x, 0, spacew, bh, spaces, 0);
                                drw_text(drw, x + spacew, 0, w - spacew, bh, c->name, 0);
				/* drw_text(drw, x, 0, w, bh, lrpad / 2 + SPCW(c), c->name, 0); */
				if (c->icon) drw_pic(drw, x, (bh - c->ich) / 2, c->icw, c->ich, c->icon);
...
setfacts(const Arg *arg) {
        ...
        for(i = 0; i < 3; i++){
                if (i == 0) {
                        areas[i].fact = TRUNC(ISINC(facts[i]) ?
                                              areas[i].fact + GETINC(facts[i]) : facts[i], 0.05, 1.95);
                }
                else {
                        areas[i].fact = TRUNC(ISINC(facts[i]) ?
                                              areas[i].fact + GETINC(facts[i])*2*(areas[i].fact) : facts[i], 0.1, 10);
                }
        }
        arrange(selmon);
}
...
{
	Layout *foo;
	foo = selmon->lt[selmon->sellt];

        /* if (!arg || !arg->v || arg->v != selmon->lt[selmon->sellt]) */
        if (!arg || !arg->v || arg->v)
                selmon->sellt = selmon->pertag->sellts[selmon->pertag->curtag] ^= 1;
        /* if (arg && arg->v) */
        if (arg && arg->v && arg->v != foo)
...
unfocus(Client *c, int setfocus)
{
        ...
        if (c == mark)
                ...
                if(c->isfloating)
                        XSetWindowBorder(dpy, c->win, scheme[SchemeNorm].floatborder->pix);
                else
                        XSetWindowBorder(dpy, c->win, scheme[SchemeNorm].border->pix);
        if (setfocus) {
                XSetInputFocus(dpy, root, RevertToPointerRoot, CurrentTime);
