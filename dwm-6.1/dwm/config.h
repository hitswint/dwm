...
static Key keys[] = {
        ...
        { WINKEY,                        XK_Up,           spawn,          SHCMD("mpc single; mpc random off; mpc repeat on; notify-send \"$(mpc | head -3 | tail -1)\"") },
        { WINKEY|ShiftMask,              XK_Up,           spawn,          SHCMD("mpc single off; mpc random; mpc repeat on; notify-send \"$(mpc | head -3 | tail -1)\"") },
